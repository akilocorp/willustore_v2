import {
  Database, Lock, Network, Search, Bluetooth, Wifi,
  Cpu, Server, ArrowDown, ArrowRight, Shield, Zap, ChevronRight
} from 'lucide-react'
...
const steps = [
  {
    ...
    icon: Database,
    color: 'accent',
    title: 'Data Vectorization',
    subtitle: 'Your AI generates embeddings',
...
await willustore.upsert({
  ...
  },
  {
    ...
    icon: Lock,
    color: 'purple',
    title: 'Fragmentation & Encryption',
    subtitle: 'Split, encrypt, never expose',
    ...
    detail: 'Critically, no single device ever holds enough fragments to reconstruct the original vector. A configurable threshold (e.g., 3 of 5 fragments) is required for reconstruction. This means partial device compromise is mathematically harmless.',
...
function fragmentAndEncrypt(vector, k=3, n=5) {
  ...
  },
  {
    ...
    icon: Network,
    color: 'green',
    title: 'Distributed Storage via Gossip',
    subtitle: 'Spread across trusted devices',
    desc: 'The central index uses a gossip protocol to discover available participating devices on your network. Encrypted fragments are distributed to devices with available storage, prioritizing proximity (same subnet, building, campus).',
    detail: 'The gossip protocol ensures fault tolerance — as devices join and leave, fragments are automatically rebalanced. The system maintains redundancy so even if 40% of participating devices go offline, all data remains retrievable.',
    code: `// Gossip protocol node discovery
class GossipNode {
  async broadcast() {
    const peers = await this.discover(); // LAN scan + seed nodes
  ...
  },
  {
    ...
    color: 'accent',
    title: 'HNSW Indexing',
    subtitle: 'Lightning-fast vector search',
...
index.add(vector_id, {
  fragment_locations: ["device:a1b2", "device:c3d4", "device:e5f6"],
  threshold: 3,
  timestamp: Date.now(),
});
...
const nearestIds = index.search(queryVector, topK=10);`,
  },
  {
    ...
    icon: Wifi,
    color: 'purple',
    title: 'Retrieval via WiFi / Bluetooth',
    subtitle: 'Local network, maximum speed',
    ...
    detail: 'This local-first retrieval model means campus or office deployments see latency under 5ms for most queries. Even in remote scenarios, the gossip protocol routes fragment requests through the most efficient available path.',
...
async function retrieveFragments(fragmentLocations) {
  ...
  },
  {
    ...
    icon: Search,
    color: 'green',
    title: 'Secure Reconstruction & Response',
    subtitle: 'Assemble, decrypt, deliver',
    desc: 'Retrieved fragments are sent to the client\'s local agent, where they are assembled using the secret sharing reconstruction algorithm and decrypted. The original vector (or the data it represents) is returned to your application — the raw data never touched any server.',
    detail: 'The entire retrieval pipeline from query to response typically completes in under 20 milliseconds for same-network deployments. All operations are audited and logged locally for compliance reporting.',
    ...
    <div className="pt-24">
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="absolute inset-0" style={{
          background: 'radial-gradient(ellipse at 50% 30%, rgba(0,212,255,0.07) 0%, transparent 60%)'
        }} />
        <div className="relative max-w-7xl mx-auto px-6 lg:px-10 text-center">
          ...
          <h1 className="font-display font-800 text-5xl md:text-6xl leading-tight mt-6 mb-6">
            Under the Hood:{' '}
            <span className="gradient-text">How Willustore Works</span>
          </h1>
          ...
          <div className="flex items-center justify-center gap-8 mt-10">
            {[
              ...
              { label: 'HNSW indexing', icon: Cpu },
              { label: 'Gossip protocol', icon: Network },
              { label: 'Local retrieval', icon: Wifi },
            ].map(({ label, icon: Icon }) => (
            ...
            <div className="flex items-center gap-3 min-w-max mx-auto w-fit">
              {['Vectorize', 'Fragment', 'Encrypt', 'Distribute', 'Index', 'Query', 'Retrieve', 'Reconstruct'].map((step, i) => (
                <div key={step} className="flex items-center gap-3">
                  ...
                  {i < 7 && <ArrowRight size={14} className="text-accent/30 mt-0" />}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <div className="space-y-16">
            {steps.map(({ num, icon: Icon, color, title, subtitle, desc, detail, code }, idx) => {
              const colors = {
                accent: { text: 'text-accent', bg: 'bg-accent/10', border: 'border-accent/20', glow: 'glow-accent' },
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <SectionHeader
            ...
            title={<>Defense in <span className="gradient-text">Depth</span></>}
            subtitle="Multiple independent layers of security ensure that even if one layer is compromised, your data remains protected."
          />
